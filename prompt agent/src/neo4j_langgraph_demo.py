...
# Neo4j connection (fallback to local defaults like the existing demo)
...
# LLM configuration (DeepSeek via OpenAI-compatible SDK or OpenAI)
...
# Prefer DashScope embeddings if available; otherwise use OpenAI embeddings.
_dashscope_key = os.getenv("DASHSCOPE_API_KEY")
if _dashscope_key:
    embeddings = DashScopeEmbeddings(model="text-embedding-v1", dashscope_api_key=_dashscope_key)
else:
    # When using OpenAIEmbeddings, avoid passing non-OpenAI base URLs to prevent 404.
...
"""
# Use parser to avoid providers requiring response_format JSON schema
...
guardrails_prompt = ChatPromptTemplate.from_messages(
    [
        ...
        (
            ...
            (
                "{question}\n\n"
                "Return JSON matching this schema strictly. Do not add backticks or text.\n"
                "{format_instructions}"
            ),
        ),
    ]
...
guardrails_parser = PydanticOutputParser(pydantic_object=GuardrailsOutput)

guardrails_chain = guardrails_prompt | llm | guardrails_parser
...
def guardrails(state: InputState) -> OverallState:
    ...
    guardrails_output = guardrails_chain.invoke({
        ...
        "format_instructions": guardrails_parser.get_format_instructions(),
    })
    database_records: Any | None = None
...
# -----------------------------
# Few-shot examples and selector
# -----------------------------
    ...
    [
        ...
        (
            ...
            (
                ...
                + "\n\nReturn JSON only that matches this schema. Do not add backticks or extra text.\n{format_instructions}"
            ),
        ),
    ]
)
...
validate_parser = PydanticOutputParser(pydantic_object=ValidateCypherOutput)

validate_cypher_chain = validate_cypher_prompt | llm | validate_parser


# Cypher query corrector is experimental
corrector_schema = [
    Schema(el["start"], el["type"], el["end"]) for el in enhanced_graph.structured_schema.get("relationships")
]
...
def validate_cypher(state: OverallState) -> OverallState:
    ...
    try:
        ...
        # neo4j Python driver exposes .message on errors
        errors.append(getattr(e, "message", str(e)))
    # Experimental feature for correcting relationship directions
    corrected_cypher = cypher_query_corrector(state.get("cypher_statement"))
    if not corrected_cypher:
        errors.append("The generated Cypher statement doesn't fit the graph schema")
    if corrected_cypher and not corrected_cypher == state.get("cypher_statement"):
    ...
    llm_output = validate_cypher_chain.invoke(
        {
            ...
            "format_instructions": validate_parser.get_format_instructions(),
        }
    )
    ...
    if llm_output.filters:
        for filter in llm_output.filters:
            ...
            node_props = enhanced_graph.structured_schema["node_props"].get(filter.node_label, [])
            matching_props = [prop for prop in node_props if prop["property"] == filter.property_key]
    ...
    return {
        ...
        "cypher_statement": corrected_cypher or state.get("cypher_statement"),
...
def _try_display_graph(compiled_graph) -> None:
    try:
        from IPython.display import Image, display

        display(Image(compiled_graph.get_graph().draw_mermaid_png()))
    except Exception:
        # Non-notebook environment, skip visualization
        pass
